...
import { SOCIAL_LOGIN_PROVIDERS } from '../model'
...
export function AuthLoginForm({ onAuthSuccess }: AuthLoginFormProps) {
  const { form, method, users, isApiLoading, onSubmit, switchMethod, fillDemoUser } = useLoginForm({
  ...
  const isUsernameFloating = isUsernameFocused || usernameValue.trim().length > 0
  const isPasswordFloating = isPasswordFocused || passwordValue.trim().length > 0
      ...
      <section className="relative mx-auto flex min-h-screen w-full max-w-4xl items-center justify-center px-4 py-8 sm:px-6">
        <div className="w-full max-w-xl rounded-[2rem] border border-[#E4E4E4] bg-[#F6F6F6]/95 px-6 py-8 shadow-[0_22px_60px_rgba(16,24,40,0.18)] backdrop-blur-sm sm:px-8">
          ...
          <div className="relative mt-6 grid grid-cols-2 rounded-2xl bg-[#E5E5E5] p-1.5">
            ...
            <button
              className={`relative z-10 h-11 rounded-xl text-lg font-semibold transition-colors duration-200 ${
                method === 'phone' ? 'text-[#111111]' : 'text-[#333333]'
              }`}
            ...
            <button
              className={`relative z-10 h-11 rounded-xl text-lg font-semibold transition-colors duration-200 ${
                method === 'email' ? 'text-[#111111]' : 'text-[#333333]'
              }`}
          ...
          <form className="mt-5 space-y-4" onSubmit={onSubmit}>
            ...
            <label className="block">
              ...
              >
                ...
                <input
                  ...
                  placeholder={method === 'phone' ? '+7 (___) ___-__-__' : 'Введите почту'}
            ...
            {rootError ? (
              <p className="rounded-xl border border-[#E9B4B4] bg-[#FFE9E9] px-3 py-2 text-sm text-[#A33E3E]">
                {rootError}
              </p>
            ) : null}
            ...
            <Button
              ...
              disabled={isApiLoading || form.formState.isSubmitting}
              size="lg"
              type="submit"
            >
              Войти
