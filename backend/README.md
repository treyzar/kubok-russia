...
### `room_templates` (миграция 000010, 000012)
| Поле | Тип | Описание |
|---|---|---|
...
7. Если статус был `new` — меняет на `starting_soon` и устанавливает `start_time = NOW() + start_delay_seconds` (динамическое значение из настроек комнаты).
...
### Нормализация структуры мест (миграция 000014)
...
- **room_places** — тесты новой реляционной структуры мест
